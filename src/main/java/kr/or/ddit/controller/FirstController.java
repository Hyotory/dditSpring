...
컴파일 및 실행 과정에서 코드를 어떻게 처리해야 할지 알려 주는 추가 정보
...
public class FirstController {
    ...
    public ModelAndView niceToMeetYou() {
        ...
        // Veiw: mustache, jsp, thymeleaf
        mav.setViewName("greetings");

        // model 객체가 "이효재" 값을 "username"속성에 연결해 웹 브라우저로 보냄
        mav.addObject("username","이효재");
        mav.addObject("myfriend","오용준");
    ...
    //URL 요청 접수
    //1. 컨트롤러는 @Controller 내부에 @GetMapping 어노테이션을 통해
    // 클라이언트의 요청을 받음
    @GetMapping("/bye")
    public String seeYouNext(Model model) {
        ...
        // mav.addObject("속성", "값")
        model.addAttribute("nickname","효재");

        // mav.setViewName("goodbye")
        // template/goodbye.mustache를 포워딩
        return "goodbye";
    }
   ...
   */
    ...
    public String newArticleForm() {
        ...
        // 뷰리졸버 조립: /templates/ + .mustache 까지 준비해줌
        // 그래서 / 와 .확장자 적지 않음.
    ...
    public String show(@PathVariable(value="id") Long id, Model model) { //매개변수로 id 받아 오기
        ...
        //  null을 저장
        // articleEntity{id=1, content=소똥, title=말똥}
        Article articleEntity = this.articleRepository.findById(id).orElse(null); // 1번 글
    ...
    public String index(Model model) {
        ...
        // LList 인터페이스의 상하 관계는 Iteratable이 가장 상위 인터페이스
    ...
    경로변수(PathVariable) : id     (entity의 기본키 id를 보통 잡음)
    요청방식 : get
     */
    //URL 주소에 있는 id를 받아오는 것이므로 데이터 타입 앞에 @PathVariable 애너테이션을 추가
    @GetMapping("/articles/{id}/edit") // PathVariable 문법이라 {} 하나만
    public String edit(@PathVariable(value = "id") Long id, Model model) {
        ...
        //뷰 페이지 설정
        // /tempates/articles/edit.mustache 포워딩
    ...
    public String delete(@PathVariable(value = "id") Long id,
        ...
        //3) 결과 페이지로 리다이렉트하기
        // 글 목록 페이지(/articles)로 리다이렉트(새로운 URI 재요청)합니다.
